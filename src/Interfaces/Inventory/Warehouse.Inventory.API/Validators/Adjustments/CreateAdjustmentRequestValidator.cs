...
public sealed class CreateAdjustmentRequestValidator : AbstractValidator<CreateAdjustmentRequest>
{
    ...
    public CreateAdjustmentRequestValidator()
    {
        ...
        RuleFor(x => x.Reason)
            .NotEmpty().WithErrorCode("INVALID_ADJUSTMENT_REASON").WithMessage("Adjustment reason is required.")
            .MaximumLength(200).WithErrorCode("INVALID_ADJUSTMENT_REASON").WithMessage("Reason must not exceed 200 characters.");
        ...
        RuleFor(x => x.Lines)
            .NotEmpty().WithErrorCode("ADJUSTMENT_LINES_REQUIRED").WithMessage("At least one adjustment line is required.");
