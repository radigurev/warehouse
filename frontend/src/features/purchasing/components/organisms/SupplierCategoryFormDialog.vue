<template>
  <FormWrapper v-model="visible" :mode="mode" max-width="500" :title="isEdit ? t('supplierCategories.edit') : t('supplierCategories.create')" :icon="isEdit ? 'mdi-tag-edit' : 'mdi-tag-plus'" @back="cancel">
    <v-card-title class="text-h6">
      {{ isEdit ? t('supplierCategories.edit') : t('supplierCategories.create') }}
    </v-card-title>
      ...
      <v-form ref="formRef" @submit.prevent="handleSubmit">
        <v-row dense>
          <v-col v-bind="grid.fieldCols">
            <v-text-field
              v-model="form.name"
              :label="t('supplierCategories.form.name')"
          ...
          <v-col v-bind="grid.fullCols">
            <v-textarea
              ...
              :label="t('supplierCategories.form.description')"
...
import { createSupplierCategory, updateSupplierCategory } from '@features/purchasing/api/supplier-categories';
...
const props = defineProps<{
  ...
  category?: SupplierCategoryDto | null;
  mode?: 'dialog' | 'page';
}>();
...
async function handleSubmit(): Promise<void> {
  ...
  try {
    if (isEdit.value && props.category) {
      await updateSupplierCategory(props.category.id, {
        name: form.name,
        description: form.description || null,
      });
      notification.success(t('supplierCategories.edit') + ' \u2713');
    } else {
      await createSupplierCategory({
        name: form.name,
        description: form.description || null,
      });
      notification.success(t('supplierCategories.create') + ' \u2713');
    }
    visible.value = false;
    emit('saved');
  } catch (err) {
